const async = require('async')
    , _ = require('lodash')
    , noble = require('noble')
    , messageParse = require('./messageParse.js')()
...
noble.on('discover', function(peripheral) {
  ...
  //log.verbose(BLE, util.inspect(peripheral, false,null));
  ...
  if(manufacturerData != null) {
      var vendorID = new Buffer(3);
      manufacturerData.copy(vendorID, 0, 0, 3);
      if (Buffer.compare(ANKIMANUFACTURERID, vendorID) !== 0) {
        log.verbose(BLE, "BTLE: Ignoring manufacturer ", vendorID);
        return;
      }
      log.verbose(BLE, "BTLE: ANKI device discover event received");
      var model_data = manufacturerData[3]
      var carName = "Unknown"
//      log.verbose(BLE, "Model Data: "+model_data);
      switch(model_data) {
//        case 1: // Kourai
//          var carName = "Kourai"
//          break;
...
//          break;
      ...
      if(carName != "Unknown") {
        ...
        log.info(BLE, "Found car: '%s', ID: '%s', Address: '%s'", carName, id, address);
...
noble.on('disconnect', function(peripheral) {
  log.verbose(BLE, "BTLE: disconnect called");
});
...
var turnOnSdkMode = function(writerCharacteristic) {
  ...
  writerCharacteristic.write(sdkMessage, false, function(err) {
    log.verbose(BLE, "Turn on SDK done.");
  });
}
...
var setLaneOffset = function(carName,change) {
  getWriterCharacteristic(carName).then(function(writerCharacteristic) {
    ...
    log.verbose(BLE, "Sending lane offset: "+change);
    writerCharacteristic.write(offsetMessage, false, function(err) {
      if(err) {
        log.verbose(BLE, "Error: "+util.inspect(err, false,null));
      } else {
        log.verbose(BLE, "Success");
      }
    });
  });
}
...
var disconnectCar = function(carName) {
  log.verbose(BLE, "Disconnect to car: "+carName);
  var peripheral = null;
  ...
  peripheral.disconnect(function(error) {
    log.verbose(BLE, "Disconnected ",error);
    // Remove the reader/writer
    log.verbose(BLE, "Reader Size: "+readerCharacteristicList.length)
    log.verbose(BLE, "Writer Size: "+readerCharacteristicList.length)
    var readerIndex = -1;
    ...
    log.verbose(BLE, "Reader Index: "+readerIndex);
    if(readerIndex > -1) {
      readerCharacteristicList.splice(readerIndex,1);
      writerCharacteristicList.splice(readerIndex,1);
    }
    log.verbose(BLE, "Reader Size After: "+readerCharacteristicList.length)
    log.verbose(BLE, "Writer Size After: "+readerCharacteristicList.length)
  });
}
...
var connectCar = function(carName) {
  log.verbose(BLE, "Connect to car: "+carName);
  ...
  var connectPromise = new Promise(
    function(resolve,reject) {
      peripheral.connect(function(error) {
        log.verbose(BLE, "Connected "+peripheral.uuid);
        peripheral.discoverServices(['be15beef6186407e83810bd89c4d8df4'], function(error, services) { // Grab write characteristic
          ...
          service.discoverCharacteristics([], function(error, characteristics) {
            ...
            log.verbose(BLE, "Characteristic Length: "+characteristics.length);
            for(var i=0;i < characteristics.length;i++) {
              log.verbose(BLE, "Looping characteristics: "+i);
              var characteristic = characteristics[i];
              if (characteristic.uuid == 'be15bee06186407e83810bd89c4d8df4') {
                log.verbose(BLE, "Adding readerChar for car" + carName);
...
function getReaderCharacteristic(carName) {
  var getReaderPromise = new Promise(
    function(resolve, reject) {
      ...
      for(var i=0; i<readerCharacteristicList.length;i++) {
        if(readerCharacteristicList[i].carName == carName) {
          log.verbose(BLE, "found reader right away "+i);
    ...
    log.verbose(BLE, "Car not connected");
    connectCar(carName).then(function(res){
      log.verbose(BLE, "In connectCar 'then'");
      for(var i=0; i<readerCharacteristicList.length;i++) {
        if(readerCharacteristicList[i].carName == carName) {
          log.verbose(BLE, "found reader after connect "+i);
          readerCharacteristic = readerCharacteristicList[i].characteristic;
          resolve(readerCharacteristic);
          return;
        }
    ...
    function(resolve, reject) {
      ...
      for(var i=0; i<writerCharacteristicList.length;i++) {
        if(writerCharacteristicList[i].carName == carName) {
          ...
          log.verbose(BLE, "resolve writer without connect.");
      ...
      connectCar(carName).then(function(res){
        ...
        for(var i=0; i<writerCharacteristicList.length;i++) {
          if(writerCharacteristicList[i].carName == carName) {
            writerCharacteristic = writerCharacteristicList[i].characteristic;
            log.verbose(BLE, "resolve writer with connect.");
            resolve(writerCharacteristic);
            return;
          }
        }
...
var setLights = function(carName,lightValue) {
  ...
  log.verbose(BLE, "set lights: Getting writer char");
  getWriterCharacteristic(carName).then(function(writerCharacteristic) {
    if(writerCharacteristic != null) {
      log.verbose(BLE, "Turn on lights");
      writerCharacteristic.write(lightMessage, false, function(err) {
        if(err) {
          log.verbose(BLE, "Error: "+util.inspect(err, false,null));
        } else {
          log.verbose(BLE, "Set LightsSuccess");
        }
      });
    }
  });
...
//  lightsPatternMessage.writeUInt16BE(cycles, 6); // cycles_per_min
  ...
  log.verbose(BLE, "set engine lights: ",lightsPatternMessage);
  getWriterCharacteristic(carName).then(function(writerCharacteristic) {
      log.verbose(BLE, "Turn on lights");
  ...
  getWriterCharacteristic(carName).then(function(writerCharacteristic) {
    ...
    writerCharacteristic.write(speedMessage, false, function(err) {
      if(err) {
        ...
        log.verbose(BLE, "Set Speed Success");
      }
    });
  });
}
...
var changeLanes = function(carName,change) {
  ...
  getWriterCharacteristic(carName).then(function(writerCharacteristic) {
    ...
    log.verbose(BLE, "Sending lane change: "+change);
      ...
      getReaderCharacteristic(carName).then(function(readerCharacteristic) {
        ...
        async.parallel([
          ...
          function(callback) { // Write the request to get battery info
            log.verbose(BLE, "running writer.");
            ...
            getWriterCharacteristic(carName).then(function(writerCharacteristic) {
              writerCharacteristic.write(message, false, function(err) {
                if(err) {
                  ...
                  log.verbose(BLE, "Request Battery Level Success");
                }
              });
              callback();
            });
          ...
          function(err) { /// Done... build reply
            log.verbose(BLE, "Battery value: "+replyData);
    ...
    function(resolve, reject) {
      getReaderCharacteristic(carName).then(function(readerCharacteristic) {
        async.parallel([
          ...
          function(callback) { // Read data until we get ping response
            ...
            function processData(data, isNotification) {
              ...
              if(messageId == 0x17) { // ANKI_VEHICLE_MSG_V2C_PING_RESPONSE
                log.verbose(BLE, "Found ping msg.");
                replyData="Success";
                readerCharacteristic.removeListener('read',processData);
                callback();
              }
          ...
          function(err) { /// Done... build reply
            log.verbose(BLE, "Ping Response: ",replyData);
            resolve(replyData);
            return;
          }
        );
...
var trackCountTravel = function(carName,tracksToTravel,speed) {
  getReaderCharacteristic(carName).then(function(readerCharacteristic){
    log.verbose(BLE, "in then after getting a reader...");
...
var mapTrack = function(carName,trackMap) {
  log.verbose(BLE, "Map Track Start...");
  ...
  getReaderCharacteristic(carName).then(function(readerCharacteristic){
    ...
    async.parallel([
      ...
      function(callback) { // Read data until we get track msg
        ...
        function processData(data, isNotification) {
          ...
          if(messageId == 0x27) { // ANKI_VEHICLE_MSG_V2C_LOCALIZATION_POSITION_UPDATE
            //log.verbose(BLE, "Position Update...");
            ...
            log.verbose(BLE, "Track Transition Event...");
            trackCount = trackCount + 1;
      ...
      function(callback) { // Write the request to start the car traveling
        log.verbose(BLE, "Starting car for mapping: "+carName);
        writerCharacteristic = getWriterCharacteristic(carName);
        setSpeed(carName,500);
        callback();
      }],
      function(err) { /// Done... build reply
        log.verbose(BLE, "Final call.  Stop car.  Mapping done.");
        log.verbose(BLE, "Starting car...");
        writerCharacteristic = getWriterCharacteristic(carName);
        setSpeed(carName,0);
      }
